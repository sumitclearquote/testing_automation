...
def draw_and_save(src_path, dest_path, label_type, threshold,test_json,data_type,car_crop):
    ...
    print(src_path)
    file_list = glob(f"{src_path}/**/*.jpg")
    folder_list = [x.rsplit("/", 2)[-2] for x in file_list]
    folder_list = set(folder_list)
    print(len(folder_list))
    file_folder_mapping = {x.rsplit("/", 2)[-1] : x.rsplit("/", 2)[-2] for x in file_list}
    print(len(file_folder_mapping))
    # print(file_folder_mapping)
    folder_count = defaultdict(int)
    for k, v in file_folder_mapping.items():
        folder_count[v] += 1
    print(folder_count)
    # print(len(file_list), file_list[:5])
    for folder in folder_list:
        # print(os.path.join(dest_path, data_type, folder, 'gt'))
        # print(os.path.join(dest_path, data_type, folder, 'pred'))
        # break
        os.system('mkdir -p '+os.path.join(dest_path, data_type, folder, 'gt'))
        os.system('mkdir -p '+os.path.join(dest_path, data_type, folder, 'pred'))
    ...
    for key in tqdm(jsn.keys()):
        ...
        #     continue
        print(filename)
        path_to_image = os.path.join(src_path,file_folder_mapping[filename], filename)
        print(path_to_image)
...
if __name__ == '__main__':
    ...
    store_name_list = ['prod_inference_amazon_47_']
    dataset_list=['amazon47']

    to_save = {j:i for i,j in zip(dataset_list,store_name_list)}
    dest = 'testing/amazon_47/'
    mask_threshold = 0.35
    car_crop = True
